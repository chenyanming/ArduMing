...
void setup() {
  ...
  // PID_YAW_ANGLE.kP(12); //David
  PID_YAW_ANGLE.kP(4);

  PID_PITCH_SPEED.kP(pitch_speed_kp);
  PID_PITCH_SPEED.kI(pitch_speed_ki);
  PID_PITCH_SPEED.imax(50);
  PID_PITCH_SPEED.kD(pitch_speed_kd);
  ...
  */
  ...
  while (con_bytesAvailable > 0) {
    ...
    if (con == '-') {
      ...
      pitch_speed_ki = pitch_speed_ki + 0.01;
      PID_PITCH_SPEED.kP(pitch_speed_ki);
    }
    else if (con == 'l') {
      pitch_speed_ki = pitch_speed_ki - 0.01;
      PID_PITCH_SPEED.kP(pitch_speed_ki);
